...
itemLoot = 0;
scr_input();
explorationAction = COMBAT;	
returnCount = 1;
function ruinsAmbient() {
	switch(explorationAction) {
		...
		case LOOT:
			...
			var log = "Loot Find!";
			itemLoot = obj_gameDatabase.ironSkin;
			...
			if (obj_Battle.combatOver == true) {
				if (obj_Player.currentLife > 0 && obj_Battle.playerRun == -1 ){
						if getCombatLoot == false {
							coinsLoot = (gettingCombatLoot(obj_Enemy.Level,obj_Enemy.Coins)+((obj_Battle.playerTurnCount+obj_Battle.enemyTurnCount)*obj_Enemy.Level))*3;
							getCombatLoot = true;
